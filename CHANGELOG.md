...
### Tests
- 263 → 289. The settings screen had no test at all and now has coverage for the cache tables it renders, the empty case, an API it cannot reach, and records missing the fields it reads. New assertions for repeated element ids across every shortcode page, the talk timing contract a theme depends on, the speaker page's caching promises, the fallback page's layout, and API records that omit a required-looking field
