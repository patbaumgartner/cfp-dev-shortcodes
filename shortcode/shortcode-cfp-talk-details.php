...
if ( ! function_exists( 'cfp_talk_details_shortcode' ) ) {
    ...
    function generate_talk_details_content($_talkId)
    {
        ...
        if (!empty($talk)) {
            ...
            $content .= '		<a class="cfp-a" href="' . cfp_dev_url("/talks-by-tracks/?id=" . $talk->trackId) . '">';
            ...
            $content .= '			<a href="' . cfp_dev_url("/talks-by-sessions/?id=" . $talk->sessionTypeId) . '">'
                . $talk->sessionTypeName . '</a> <em>(' . $talk->audienceLevel . ' level)</em>';
            $content .= '       </div>';
            ...
            foreach ($semanticResult as $item) {
                if ($item->id != $talk->id && strpos(strtolower($item->title), 'overflow') === false) {
                    ...
                    if (get_option('cfp_dev_content_by_id', 'yes') == 'no') {
                        $content .= '       <a href="' . cfp_dev_url("/talk/" . generate_slug($item->title)) . '">' . $item->title . '</a>';
                    } else {
                        $content .= '       <a href="' . cfp_dev_url("/talk?id=$item->id") . '">' . $item->title . '</a>';
                    }
                    $content .= '    </div>';
                }
            }
